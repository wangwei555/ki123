...
Rectangle {                                           // 矩形根元素
    ...
    Emitter {                                                  // 这就是我们的发射器了
        ...
        y: mouseArea.pressed ?
 mouseArea.mouseY : circle.cy       // 发射到的坐标值
        x: mouseArea.pressed ? mouseArea.mouseX : circle.cx       // 这里使用mouseArea检測是否按下。有则使用按下的坐标，否则使用以下的计算坐标。这里的粒子之所以能够持续的发射，其缘由正是QML的属性绑定，由于这个坐标值的持续变化。造成了我们所见的粒子动画。
 ...
 mouseArea.mouseY : circle2.cy
    ...
    color: "white"                                                     // 不明确这个color的意义，由于矩形的颜色在渐变那里已经被确定了
